...
│   ├── handlers/            # /api/* HTTP handlers（按 module 分文件）+ 集中错误处理器
...
- 集中错误处理器（§7）保证所有错误——含 Echo 内部 404/405 与 panic——都转为信封，而非 Echo 默认响应。
...
- 错误码（`UPPER_SNAKE_CASE`，带模块前缀）：
  ...
  - `NOT_FOUND`（workspace/project/service/database/build/run/dependency 缺失；亦用于路由 404）
  - `METHOD_NOT_ALLOWED`（HTTP 405）
  - `INTERNAL_ERROR`（系统层，不向终端暴露细节）
- 校验：请求 struct + `go-playground/validator`，失败 → `VALIDATION_ERROR` + `details`。
- **集中错误处理器**：Echo 的 `HTTPErrorHandler`（`internal/handlers/errorhandler.go`）将所有错误转为信封。handler 失败路径 `return err`（成功路径 `c.JSON(200, OK)`），错误统一经此处理器出口：
  - 业务错误 `*apperrors.Error` → HTTP **200** + `Fail(err)`。
  - 传输/系统错误（实现 `echo.HTTPStatusCoder`，含 `*echo.HTTPError` 与 sentinel `ErrNotFound`/`ErrMethodNotAllowed` 等）→ 对应 4xx/5xx + 信封；404→`NOT_FOUND`、405→`METHOD_NOT_ALLOWED`、其余→`INTERNAL_ERROR`。
  - 未知错误与 panic（经 `middleware.Recover` 恢复）→ 500 + `INTERNAL_ERROR`。
  - `message` 用固定串，不泄露内部细节。
